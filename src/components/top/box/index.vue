    ...
    <div class="container" @click.stop>
        <div class="header">
            <div class="header_img fl">
                <img :src="`${this.$fixImg(singer.head_info ? singer.head_info.link : singer.head_link, `w=105&h=120`)}`">
            </div>
            <div class="text fr">
                <p class="total_list">
                    <!-- <span>查看总榜></span> -->
        ...
        <ul class="list" v-if="infos.data_list.length">
            <li class="clear_float" v-for="(item, index) in infos.data_list">
                ...
                <header-img :size="80" :header-img="item.head_info ? item.head_info.link : item.head_link" :header-id="item.uid" :vip="item.is_music == 3"></header-img>
