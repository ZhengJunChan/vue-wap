...
<template>
  <div id="topList" class="ranking_list_page" :class="{'wei_xin': $parent.isFromWX, top34: Number(active.class > 2)}">
    ...
    <div id="list" class="list" v-if="Number(active.class <= 2)">
...
import RankListgApi from './rank-list-api.js';

import TopList from './component/top-list.vue';
import TopSinger from './component/top-singer.vue';
    ...
    methods: {
        ...
        getList(type) {
            ...
            RankListgApi.getList(params).then(res => {
                ...
                this.setAppParams(params.class_id, params.type, this.list.billboard_type, this.list.toggle_info.id);
            }, error => {
                this.$toast.error(error.msg);
            });
        },
        setAppParams(classId, typeId, billboardType, toggleId) {
            this.$route.meta.appParams.class = classId || this.active.listId;
            this.$route.meta.appParams.billboard_type = billboardType || this.active.class;
            this.$route.meta.appParams.type = typeId || this.active.type;
            this.$route.meta.appParams.toggle_id = toggleId || '';
        },
